...
from db.shared_repositories import tags_repository
...
class TagsRepositoryTestCase(unittest.TestCase):
    def test_create_tag(self):
        ...
        with tags_repository.create_session() as session:
            session.create(tag)
            retrieved_tag = session.get_first({ "id": "test_id_create" })
    ...
    def test_delete_tag(self):
        ...
        with tags_repository.create_session() as session:
            session.create(tag)
            session.delete(tag)
            retrieved_tag = session.get_first({ "id": "test_id_delete" })
            self.assertIsNone(retrieved_tag)
            # Ensure the tag is deleted
            all_tags = session.list()
